...
angular.module('phoneLocator').controller('FinderCtrl', function($scope, phoneService, httpExtension, errorReporter, logger) {
    ...
    $scope.appState = $scope.AppStates.DEFAULT;
     ...
     $scope.sendEmail = function(emailAddress, opt_subject, opt_body) {
        ...
        // The basic flow is:
        // -Open a new tab with the email URL
        // -Give it 500ms to open outlook (it needs a bit of asynchronous time to do its thing)
        // Close the tab.
     ...
     $scope.onMorePressed = function(person) {
        ...
        if(person.showMore) {
            ...
            window.location.hash = '#' + person.mispar_ishi;
        }
        else {
            $scope.showMoreClass = "";
    ...
    function setMeAsPerson(meAsPerson) {
        ...
        chrome.storage.sync.set({'meAsPerson': meAsPerson});
    };
...
angular.module('phoneLocator').service('errorReporter', function(httpExtension) {
    ...
    function sendError(newError) {   
        ...
        httpExtension.sendPost(requestObject).error(function() {
              chrome.storage.sync.set({'ERROR': newError});
        });
    };

    return {
        onInitialization: function() {
            chrome.storage.sync.get('ERROR', function(errorsObject) {
                var error = errorsObject['ERROR'];
                if (!error) {
                    return;
                }
                chrome.storage.sync.set({'ERROR': ''}, function() {
                    sendError(error);
                });
            });
        },
